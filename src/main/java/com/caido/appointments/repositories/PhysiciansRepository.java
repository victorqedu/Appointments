...
public interface PhysiciansRepository extends JpaRepository<Physicians, Integer> {
    @Query("SELECT NEW com.caido.appointments.entity.DTO.SimplePhysicianDTO(p.id, p.stencilNo, p.idPersonnel.imagineAngajat, p.idPersonnel.idperson.name, p.idPersonnel.idperson.surname) "
            + "FROM Physicians p "
