...
public interface PhysiciansRepository extends JpaRepository<Physicians, Integer> {
    @Query("SELECT NEW com.caido.appointments.entity.DTO.SimplePhysicianDTO(p.id, p.stencilNo, p.idPersonnel.imagineAngajat, p.idPersonnel.idperson.name, p.idPersonnel.idperson.surname, pr.idSex, p.idPersonnel.id, d.id) "
            + "FROM Physicians p "
            ...
            + "GROUP BY p.id, p.stencilNo, p.idPersonnel.imagineAngajat, p.idPersonnel.idperson.name, p.idPersonnel.idperson.surname, pr.idSex, p.idPersonnel.id, d.id "
            + "ORDER BY p.id DESC")
