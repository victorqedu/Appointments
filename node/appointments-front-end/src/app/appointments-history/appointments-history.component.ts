...
export class AppointmentsHistoryComponent {
  ...
  ngOnInit(): void {
    console.log("AppointmentsHistoryComponent.ngOnInit");
    this.fetchData();
  }
  ...
  fetchData() {
    ...
    if(this.accountService.getAccount().id!==null && this.accountService.getAccount().cnp!==null && this.accountService.getAccount().cnp!=="0000000000000") {
      ...
      this.httpService.getPatientAppointmetns(this.recPerPage, (this.currentPage-1)*this.recPerPage).subscribe(appointments => {
        console.log(appointments);
        this.appointments = appointments as Appointment[];
        this.isFetching = false;
      });
      this.httpService.countConnectedUserAppointments().subscribe(total => {
